...
struct EnemyParams {
    ...
    int baseHealth = 100;
    std::string texturePath = "assets/images/enemies/default_enemy.png"; // 追加
    ...
    friend void from_json(const json& j, EnemyParams& p) {
        ...
        if (j.contains("texturePath")) j.at("texturePath").get_to(p.texturePath); // 追加
    }
};
